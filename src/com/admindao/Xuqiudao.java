...
public class Xuqiudao {
	...
	public List<Zhengjibean> fenleisearch(int shendu, String name) {
		List <Zhengjibean> list = null;
		Session session = HibernateUtils.getCurrentSession();
		if(shendu==1&&name.equals("1"))
		{
			Query query = session.createQuery("from Zhengjibean where kejihuodongleixing = ?");
			query.setString(0, "基础研究");
			list = query.list();
		}
		if(shendu==1&&name.equals("2"))
		{
			Query query = session.createQuery("from Zhengjibean where kejihuodongleixing != ?");
			query.setString(0, "基础研究");
			list = query.list();
