$(function(){
 $("#demo2").easyinsert({
  ...
  type: ["select","custom","select","custom", "text","custom", "text"],
  value: [{ '并含': 'and', '或含': 'or', '不含': 'and not' },"->", { '通过日期': 'passtime', '公布日期': 'publishtime', '施行日期': 'applytime' },"：从", "", "到",""],//可以给同组input分别设置默认值
  className: ["", "", "","","inputxt","","inputxt"],//不用我解释了吧
 ...
 $("#demo5").easyinsert({//type新增custom和select
  ...
  type: ["select","custom" ,"select","custom", "text","custom", "select"],
  value: [{ '并含': 'and', '或含': 'or', '不含': 'and not' },"->",{ '政策名称': 'pname', '政策分类': 'pcategories', '施行范围': 'pboundary', '颁布形式': 'pBBXS', '序号或文号': 'pno','制定机关': 'poffice','调整领域': 'field', '主题词': 'BZTC', '关键字': 'BGJC' },":&nbsp;", "","&nbsp;", { '精确查询': '1', '模糊查询': '2'}],
  className: ["","","","","inputxt","", ""],
  toplimit: 9//可以添加组数上限（默认是0，表示无上限），它是总管，so，name相当于小组组长
 });
});
;(function($){
 $.fn.extend({
