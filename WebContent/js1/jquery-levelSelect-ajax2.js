...
$.openLayer2 = function(p){
	var param = $.extend({
		...
		title : "检索：<input type='text' id='search'>",				//弹出窗口标题
		width : 650,					//弹出窗口宽度
	...
	var fs = {
		...
		init_area : function(){			//初始化数据容器
			...
			//已选择项容器
			var selArea = $("<div id='selArea' style=' overflow-y:scroll'><div>已选择项目：</div></div>");
			_container.append(selArea); 
			if (param.maxItems == 1){ selArea.hide(); }

			//初始化第一层级数据容器，以后各级容器都clone本容器
			var d1 = $("<div id='d1' style=' overflow-y:scroll' ></div>");
		...
		add_data : function(targetid){					//添加数据到容器，添加事件，初始化下一层次容器
			...
			for(var i = 0 ; i < dat.length ; i++){
				...
				html.push("<span title='"+dat[i]+"' name='"+pid+"' style='overflow: hidden;width:"+span_width+"px;'>");
				if(x!=0){
					html.push("<br>");
				}
				x=1;
			...
			$("#search").keyup(function(){
				...
				setTimeout(function(){
					...
					for (var i = 0; i < zc.zc.length; i++) {
						var reg = new RegExp('^' + $("#search").val() + '.*$', 'im');
						if (reg.test(zc.zc[i].ZCMC) ) {
							
							var span_width = eval("param.span_width."+$("#d1").attr("id"));			//每个数据显示项的宽度
							span_width = (span_width == undefined ? param.span_width.d1:span_width );//没有设置的话，就使用第一个数据容器的值
							var inspan_width = ($.browser.msie)?1:3;								//内部文字和checkbox之间的距离
												//根据设定分隔符对数据做第一次分隔，获得数据项数组
																			//格式化数据存放容器，为了提高效率，使用了数组
							
							
							html.push("<span title='"+zc.zc[i]+"' name='"+pid+"' style='width:"+span_width+"px;'>");
							if(x!=0){
								html.push("<br>");
							}
							x=1;
							html.push("<input type='checkbox' value='" + zc.zc[i].ZCWH + "'>");
							html.push("<span name='"+$("#d1").attr("id")+"' style='margin-left:"+inspan_width+"px;'>" + zc.zc[i].ZCMC + "</span>");
							html.push("</span>");
						}
						
					}
					$("#d1").empty().show().append($(html.join("")));
					if(param.maxItems > 1){fs.change_status(targetid);}		//同步状态,单选状态无必要
					
					fs.add_input_event($("#d1"));							//加入input的事件绑定
					fs.add_span_event($("#d1"));
	...
	//			if (maxlev > thislevel){
		...
		init_style : function() {	//初始化css
			...
			css.push("#selArea {width:"+_width+"px;height:100px;margin:"+_margin+"px;padding:5px;background-color:#f4f4f4;float:left;"+aotu+"}");
			...
			var d_css = "{width:"+_width+"px;margin:"+_margin+"px;padding:5px;height:150px;;background-color:khaki;float:left;"+aotu+"}";
			css.push("dc "+d_css);
